...
app.get('/',(req,res)=>res.render('menu',{title:'Menu'}));

app.get('/all', (req, res) => storage.getAll()
    .then(data => res.render('allComputers',{title:'Computers',result:data}))
    .catch(err => res.json(err)));

app.get('/getone', (req, res) => res.render('form', {
    title: 'Computers',
    header: 'Input id:',
    action: '/getone',
    fields: [
...
app.post('/getone', (req, res) =>
    storage.getOne(req.body.id)
        .then(data => res.render('oneComputer',{title:'Computers',computer:data}))
...
app.post('/update', (req,res)=>storage.getOne(req.body.id)
    .then(data=>res.render('form',{
        title: 'Computers',
        header: 'Fill in the Data',
        action: '/updatedata',
        fields: [
...
app.get('/removeform', (req,res)=>res.render('form',{
    title: 'Computers',
    header: 'Input id:',
    action: '/remove',
    fields: [
...
function sendErrorPage(res,err){
    sendStatusPage(res,err,'Error','Error');
}
